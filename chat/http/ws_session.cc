...
chat::ConfigVar<uint32_t>::ptr g_websocket_message_max_size
    = chat::Config::Lookup("websocket.message.max_size"
...
HttpRequest::ptr WSSession::handleShake() {
    ...
    do {
        req = recvRequest();
        if(!req) {
            CHAT_LOG_INFO(g_logger) << "invalid http request";
            break;
        }
        if(strcasecmp(req->getHeader("Upgrade").c_str(), "websocket")) {
            CHAT_LOG_INFO(g_logger) << "http header Upgrade != websocket";
            break;
        }
        if(strcasecmp(req->getHeader("Connection").c_str(), "Upgrade")) {
            CHAT_LOG_INFO(g_logger) << "http header Connection != Upgrade";
            break;
        }
        if(req->getHeaderAs<int>("Sec-webSocket-Version") != 13) {
            CHAT_LOG_INFO(g_logger) << "http header Sec-webSocket-Version != 13";
            break;
        }
        std::string key = req->getHeader("Sec-WebSocket-Key");
        if(key.empty()) {
            CHAT_LOG_INFO(g_logger) << "http header Sec-WebSocket-Key = null";
            break;
        }

        std::string v = key + "258EAFA5-E914-47DA-95CA-C5AB0DC85B11";
        v = chat::base64encode(chat::sha1sum(v));
        ...
        CHAT_LOG_DEBUG(g_logger) << *rsp;
        return req;
    } while(false);
    if(req) {
        CHAT_LOG_INFO(g_logger) << *req;
    }
    return nullptr;
}
    ...
    do {
        ...
        if(ws_head.opcode == WSFrameHead::PING) {
            ...
            if(ws_head.payload == 126) {
                ...
                length = chat::byteswapOnLittleEndian(len);
            ...
            if((cur_len + length) >= g_websocket_message_max_size->getValue()) {
                CHAT_LOG_WARN(g_logger) << "WSFrameMessage length > "
...
int32_t WSSendMessage(Stream* stream, WSFrameMessage::ptr msg, bool client, bool fin) {
    do {
        ...
        if(ws_head.payload == 126) {
            uint16_t len = size;
            len = chat::byteswapOnLittleEndian(len);
            if(stream->writeFixSize(&len, sizeof(len)) <= 0) {
                break;
            }
        } else if(ws_head.payload == 127) {
            uint64_t len = chat::byteswapOnLittleEndian(size);
            if(stream->writeFixSize(&len, sizeof(len)) <= 0) {
                break;
            }
        }
