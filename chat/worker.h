...
public:
    ...
    static TimedWorkerGroup::ptr Create(uint32_t batch_size, uint32_t wait_ms, chat::IOManager* s = chat::IOManager::GetThis());

    TimedWorkerGroup(uint32_t batch_size, uint32_t wait_ms, chat::IOManager* s = chat::IOManager::GetThis());
    ~TimedWorkerGroup();
