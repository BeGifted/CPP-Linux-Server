...
Socket::ptr Socket::CreateTCP(chat::Address::ptr address) {
    Socket::ptr sock = std::make_shared<Socket>(address->getFamily(), TCP, 0);
    return sock;
}

Socket::ptr Socket::CreateUDP(chat::Address::ptr address) {
...
bool Socket::getOption(int level, int option, void* result, socklen_t* len) {
    ...
    if(rt) {
        CHAT_LOG_DEBUG(g_logger) << "getOption sock=" << m_sock
...
bool Socket::setOption(int level, int option, const void* result, socklen_t len) {
    if(setsockopt(m_sock, level, option, result, len)) {
        CHAT_LOG_DEBUG(g_logger) << "setOption sock=" << m_sock
            << " level=" << level << " option=" << option
            << " errno=" << errno << " errstr=" << strerror(errno);
        return false;
    }
...
Socket::ptr Socket::accept() {
    ...
    int newsock = ::accept(m_sock, nullptr, nullptr);  //使用全局accept
    if (newsock == -1) {
        CHAT_LOG_ERROR(g_logger) << "accept(" << m_sock << ") errno="
...
bool Socket::bind(const Address::ptr addr) {
    ...
    if (CHAT_UNLIKELY(addr->getFamily() != m_family)) {
        CHAT_LOG_ERROR(g_logger) << "bind sock.family("
...
bool Socket::reconnect(uint64_t timeout_ms) {
    if(!m_remoteAddress) {
        CHAT_LOG_ERROR(g_logger) << "reconnect m_remoteAddress is null";
        return false;
    }
...
bool Socket::connect(const Address::ptr addr, uint64_t timeout_ms) {
    ...
    if (CHAT_UNLIKELY(addr->getFamily() != m_family)) {
        CHAT_LOG_ERROR(g_logger) << "connect sock.family("
            << m_family << ") addr.family(" << addr->getFamily()
            << ") not equal, addr=" << addr->toString();
        return false;
    }
    ...
    if(timeout_ms == (uint64_t)-1) {
        if(::connect(m_sock, addr->getAddr(), addr->getAddrLen())) {
            CHAT_LOG_ERROR(g_logger) << "sock=" << m_sock << " connect(" << addr->toString()
...
bool Socket::listen(int backlog) {
    if (!isValid()) {
        CHAT_LOG_ERROR(g_logger) << "listen error sock=-1";
        return false;
    }
    if (::listen(m_sock, backlog)) {
        CHAT_LOG_ERROR(g_logger) << "listen error errno=" << errno
            << " errstr=" << strerror(errno);
        return false;
    }
    return true;
...
Address::ptr Socket::getLocalAddress() {
    ...
    socklen_t addrlen = result->getAddrLen();
    if(getsockname(m_sock, result->getAddr(), &addrlen)) {  //获取套接字本地地址
        CHAT_LOG_ERROR(g_logger) << "getsockname error sock=" << m_sock
            << " errno=" << errno << " errstr=" << strerror(errno);
        return std::make_shared<UnknownAddress>(m_family);
    }
    if(m_family == AF_UNIX) {
...
bool Socket::cancelRead() {
    return IOManager::GetThis()->cancelEvent(m_sock, chat::IOManager::READ);
}

bool Socket::cancelWrite() {
    return IOManager::GetThis()->cancelEvent(m_sock, chat::IOManager::WRITE);
}
...
void Socket::newSock() {
    ...
    if (CHAT_LIKELY(m_sock != -1)) {
        initSock();
    } else {
        CHAT_LOG_ERROR(g_logger) << "socket(" << m_family
...
bool SSLSocket::loadCertificates(const std::string& cert_file, const std::string& key_file) {
    ...
    if(SSL_CTX_use_certificate_chain_file(m_ctx.get(), cert_file.c_str()) != 1) {
        CHAT_LOG_ERROR(g_logger) << "SSL_CTX_use_certificate_chain_file("
            << cert_file << ") error";
        return false;
    }
    if(SSL_CTX_use_PrivateKey_file(m_ctx.get(), key_file.c_str(), SSL_FILETYPE_PEM) != 1) {
        CHAT_LOG_ERROR(g_logger) << "SSL_CTX_use_PrivateKey_file("
            << key_file << ") error";
        return false;
    }
    if(SSL_CTX_check_private_key(m_ctx.get()) != 1) {
        CHAT_LOG_ERROR(g_logger) << "SSL_CTX_check_private_key cert_file="
...
SSLSocket::ptr SSLSocket::CreateTCP(chat::Address::ptr address) {
    SSLSocket::ptr sock = std::make_shared<SSLSocket>(address->getFamily(), TCP, 0);
    return sock;
}

SSLSocket::ptr SSLSocket::CreateTCPSocket() {
    SSLSocket::ptr sock = std::make_shared<SSLSocket>(IPv4, TCP, 0);
    return sock;
}

SSLSocket::ptr SSLSocket::CreateTCPSocket6() {
    SSLSocket::ptr sock = std::make_shared<SSLSocket>(IPv6, TCP, 0);
    return sock;
}
