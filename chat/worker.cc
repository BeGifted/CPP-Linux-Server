...
TimedWorkerGroup::ptr TimedWorkerGroup::Create(uint32_t batch_size, uint32_t wait_ms, chat::IOManager* s) {
...
void TimedWorkerGroup::start() {
    m_timer = chat::IOManager::GetThis()->addTimer(m_waitTime, std::bind(&TimedWorkerGroup::onTimer, shared_from_this()));
}
...
TimedWorkerGroup::TimedWorkerGroup(uint32_t batch_size, uint32_t wait_ms, chat::IOManager* s)
    :m_batchSize(batch_size)
    ,m_finish(false)
    ,m_timedout(false)
    ,m_waitTime(wait_ms)
