...
GrpcConnection::GrpcConnection() {
    CHAT_LOG_INFO(g_logger) << "GrpcConnection::GrpcConnection sock=" << m_socket << " - " << this;
}
...
GrpcResponse::ptr GrpcConnection::request(GrpcRequest::ptr req, uint64_t timeout_ms) {
    ...
    CHAT_LOG_DEBUG(g_logger) << "send data.size=" << data.size();
    if(!result->result) {
        rsp->setResult(chat::TypeUtil::Atoi(result->response->getHeader(GRPC_STATUS)));
        ...
        if(result->response) {
            ...
            if(!result->response->getBody().empty()) {
                ...
                if(DecodeGrpcBody(result->response->getBody(), *msg)) {
                    ...
                    CHAT_LOG_WARN(g_logger) << "recv grpc response fail, DecodeGrpcBody, body.size="
