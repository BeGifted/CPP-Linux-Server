...
Http2SocketStream::Http2SocketStream(Socket::ptr sock, bool client)
    ...
    CHAT_LOG_INFO(g_logger) << "Http2SocketStream::Http2SocketStream sock=" << sock << " - " << this;
}

Http2SocketStream::~Http2SocketStream() {
    CHAT_LOG_INFO(g_logger) << "Http2SocketStream::~Http2SocketStream " << this;
}

void Http2SocketStream::onClose() {
    CHAT_LOG_INFO(g_logger) << "******** onClose " << getLocalAddressString() << " - " << getRemoteAddressString();
    m_streamMgr.clear();
}

bool Http2SocketStream::handleShakeClient() {
    CHAT_LOG_INFO(g_logger) << "handleShakeClient " << getRemoteAddressString();
    if(!isConnected()) {
        return false;
    }
...
void Http2SocketStream::handleRecvData(Frame::ptr frame, Http2Stream::ptr stream) {
    if(frame->header.length) {
        ...
        if(m_recvWindow < (int32_t)MAX_INITIAL_WINDOW_SIZE / 4) {
            CHAT_LOG_INFO(g_logger) << "recv_window=" << m_recvWindow
        ...
        if(stream->m_recvWindow < (int32_t)MAX_INITIAL_WINDOW_SIZE / 4) {
            CHAT_LOG_INFO(g_logger) << "recv_window=" << stream->m_recvWindow
                << " length=" << frame->header.length
                << " " << (MAX_INITIAL_WINDOW_SIZE / 4)
...
struct ScopeTest {
    ScopeTest() {
        CHAT_LOG_INFO(g_logger) << "=========== DoRecv ===========";
    }
    ~ScopeTest() {
        CHAT_LOG_INFO(g_logger) << "=========== DoRecv Out ===========";
    }
};
...
AsyncSocketStream::Ctx::ptr Http2SocketStream::doRecv() {
    ...
    CHAT_LOG_DEBUG(g_logger) << getRemoteAddressString() << " recv: " << frame->toString();
...
//        CHAT_LOG_INFO(g_logger) << "send response ======";
...
int32_t Http2SocketStream::sendData(Http2Stream::ptr stream, const std::string& data, bool async, bool end_stream) {
    ...
    do {
        ...
        if(rt <= 0) {
            CHAT_LOG_DEBUG(g_logger) << "sendData error rt=" << rt << " errno=" << errno << " - " << getRemoteAddressString();
            return rt;
        }
        length -= len;
        pos += len;
        ...
        chat::Atomic::addFetch(m_sendWindow, -len);
        //send_window -= len;
    } while(length > 0);
    return 1;
}
...
bool Http2SocketStream::StreamCtx::doSend(AsyncSocketStream::ptr stream) {
    ...
    auto stm = h2stream->getStream(sn);
    if(!stm) {
        CHAT_LOG_ERROR(g_logger) << "StreamCtx doSend Fail, sn=" << sn
            << " not exists - " << stream->getRemoteAddressString();
        return false;
    }
...
int32_t Http2SocketStream::sendWindowUpdate(uint32_t stream_id, uint32_t n) {
    //CHAT_LOG_INFO(g_logger) << "----sendWindowUpdate id=" << stream_id << " n=" << n;
    Frame::ptr frame = std::make_shared<Frame>();
