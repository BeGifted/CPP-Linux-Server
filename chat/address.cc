...
Address::ptr Address::Create(const sockaddr* addr, socklen_t addrlen) {
    ...
    switch(addr->sa_family) {
        case AF_INET:
            result = std::make_shared<IPv4Address>(*(const sockaddr_in*)addr);  //sockaddr要转换类型
            break;
        case AF_INET6:
...
bool Address::Lookup(std::vector<Address::ptr>& result, const std::string& host, int family, int type, int protocol) {
    ...
    if (error) {
        CHAT_LOG_DEBUG(g_logger) << "Address::Lookup getaddress(" << host << ", "
    ...
    while(next) {
        ...
        //CHAT_LOG_INFO(g_logger) << ((sockaddr_in*)next->ai_addr)->sin_addr.s_addr;
        next = next->ai_next;
    }
...
bool Address::GetInterfaceAddresses(std::multimap<std::string, std::pair<Address::ptr, uint32_t> >& result, int family) {
    ...
    if (getifaddrs(&results) != 0) {
        CHAT_LOG_DEBUG(g_logger) << "Address::GetInterfaceAddresses getifaddrs "
    ...
    try {
        ...
        CHAT_LOG_ERROR(g_logger) << "Address::GetInterfaceAddresses exception";
        freeifaddrs(results);
        return false;
    }
    freeifaddrs(results);
...
bool Address::operator<(const Address& rhs) const {
    ...
    int res = memcmp(getAddr(), rhs.getAddr(), minlen);  //比较内存区域
...
IPAddress::ptr IPAddress::Create(const char* address, uint16_t port) {
    ...
    hints.ai_flags = AI_CANONNAME;  //将 address 参数解释为一个数值地址，而不是一个主机名
    hints.ai_family = AF_UNSPEC;  //v4/v6

    int error = getaddrinfo(address, NULL, &hints, &results);  //解释为一个数值地址
    if (error) {
        CHAT_LOG_DEBUG(g_logger) << "IPAddress::Create(" << address
            << ", " << port << ") error=" << error
...
IPv4Address::ptr IPv4Address::Create(const char* address, uint16_t port) {
    ...
    if(result <= 0) {
        CHAT_LOG_DEBUG(g_logger) << "IPv4Address::Create(" << address << ", "
...
std::ostream& IPv4Address::insert(std::ostream& os) const {
    //inet_ntoa/inet_ntop(网络字节序表示的v4转化为点分十进制)
...
IPv6Address::ptr IPv6Address::Create(const char* address, uint16_t port) {
    ...
    if(result <= 0) {
        CHAT_LOG_DEBUG(g_logger) << "IPv6Address::Create(" << address << ", "
                << port << ") rt=" << result << " errno=" << errno
                << " errstr=" << strerror(errno);
        return nullptr;
    }
...
IPv6Address::IPv6Address(const uint8_t address[16], uint16_t port) {
    ...
    memcpy(&m_addr.sin6_addr.s6_addr, byteswapOnLittleEndian(address), 16);
}
