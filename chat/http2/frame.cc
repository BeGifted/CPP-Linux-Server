...
bool FrameHeader::writeTo(ByteArray::ptr ba) {
    try {
        ...
        CHAT_LOG_WARN(g_logger) << "write FrameHeader fail, " << toString();
    }
    return false;
}
...
bool FrameHeader::readFrom(ByteArray::ptr ba) {
    try {
        ...
        CHAT_LOG_WARN(g_logger) << "read FrameHeader fail, " << toString();
    }
    return false;
}
...
bool DataFrame::writeTo(ByteArray::ptr ba, const FrameHeader& header) {
    try {
        ...
        CHAT_LOG_WARN(g_logger) << "write DataFrame fail, " << toString();
    }
    return false;
}
...
bool DataFrame::readFrom(ByteArray::ptr ba, const FrameHeader& header) {
    try {
        ...
        CHAT_LOG_WARN(g_logger) << "read DataFrame fail, " << toString();
    }
    return false;
}
...
bool PriorityFrame::writeTo(ByteArray::ptr ba, const FrameHeader& header) {
    try {
        ...
        CHAT_LOG_WARN(g_logger) << "write PriorityFrame fail, " << toString();
    }
    return false;
}
...
bool PriorityFrame::readFrom(ByteArray::ptr ba, const FrameHeader& header) {
    try {
        ...
        CHAT_LOG_WARN(g_logger) << "read PriorityFrame fail, " << toString();
    }
    return false;
}
...
bool HeadersFrame::writeTo(ByteArray::ptr ba, const FrameHeader& header) {
    try {
        ...
        CHAT_LOG_WARN(g_logger) << "write HeadersFrame fail, " << toString();
    }
    return false;
}
...
bool HeadersFrame::readFrom(ByteArray::ptr ba, const FrameHeader& header) {
    try {
        ...
        CHAT_LOG_WARN(g_logger) << "read HeadersFrame fail, " << toString();
    }
    return false;
}
...
bool RstStreamFrame::writeTo(ByteArray::ptr ba, const FrameHeader& header) {
    try {
        ...
        CHAT_LOG_WARN(g_logger) << "write RstStreamFrame fail, " << toString();
    }
    return false;
}
...
bool RstStreamFrame::readFrom(ByteArray::ptr ba, const FrameHeader& header) {
    try {
        ...
        CHAT_LOG_WARN(g_logger) << "read RstStreamFrame fail, " << toString();
    }
    return false;
}
...
bool SettingsFrame::writeTo(ByteArray::ptr ba, const FrameHeader& header) {
    try {
        ...
        CHAT_LOG_WARN(g_logger) << "write SettingsFrame fail, " << toString();
    }
    return false;
}
...
bool SettingsFrame::readFrom(ByteArray::ptr ba, const FrameHeader& header) {
    try {
        ...
        CHAT_LOG_WARN(g_logger) << "read SettingsFrame fail, " << toString();
    }
    return false;
}
...
bool PushPromisedFrame::writeTo(ByteArray::ptr ba, const FrameHeader& header) {
    try {
        ...
        CHAT_LOG_WARN(g_logger) << "write PushPromisedFrame fail, " << toString();
    }
    return false;
}
...
bool PushPromisedFrame::readFrom(ByteArray::ptr ba, const FrameHeader& header) {
    try {
        ...
        CHAT_LOG_WARN(g_logger) << "read PushPromisedFrame fail, " << toString();
    }
    return false;
}
...
bool PingFrame::writeTo(ByteArray::ptr ba, const FrameHeader& header) {
    try {
        ...
        CHAT_LOG_WARN(g_logger) << "write PingFrame fail, " << toString();
    }
    return false;
}
...
bool PingFrame::readFrom(ByteArray::ptr ba, const FrameHeader& header) {
    try {
        ...
        CHAT_LOG_WARN(g_logger) << "read PingFrame fail, " << toString();
    }
    return false;
}
...
bool GoAwayFrame::writeTo(ByteArray::ptr ba, const FrameHeader& header) {
    try {
        ...
        CHAT_LOG_WARN(g_logger) << "write GoAwayFrame fail, " << toString();
    }
    return false;
}
...
bool GoAwayFrame::readFrom(ByteArray::ptr ba, const FrameHeader& header) {
    try {
        ...
        CHAT_LOG_WARN(g_logger) << "read GoAwayFrame fail, " << toString();
    }
    return false;
}
...
bool WindowUpdateFrame::writeTo(ByteArray::ptr ba, const FrameHeader& header) {
    try {
        ...
        CHAT_LOG_WARN(g_logger) << "write WindowUpdateFrame fail, " << toString();
    }
    return false;
}
...
bool WindowUpdateFrame::readFrom(ByteArray::ptr ba, const FrameHeader& header) {
    try {
        ...
        CHAT_LOG_WARN(g_logger) << "read WindowUpdateFrame fail, " << toString();
    }
    return false;
}
...
Frame::ptr FrameCodec::parseFrom(Stream::ptr stream) {
    try {
        ...
        if(rt <= 0) {
            CHAT_LOG_INFO(g_logger) << "recv frame header fail, rt=" << rt << " " << strerror(errno);
            return nullptr;
        }
        ba->setPosition(0);
        if(!frame->header.readFrom(ba)) {
            CHAT_LOG_INFO(g_logger) << "parse frame header fail";
            return nullptr;
        }
        if(frame->header.length > 0) {
            ...
            if(rt <= 0) {
                CHAT_LOG_INFO(g_logger) << "recv frame body fail, rt=" << rt << " " << strerror(errno);
                return nullptr;
            }
            ba->setPosition(0);
        }
        switch((FrameType)frame->header.type) {
            case FrameType::DATA:
                {
                    ...
                    if(!frame->data->readFrom(ba, frame->header)) {
                        CHAT_LOG_INFO(g_logger) << "parse DataFrame fail";
                        return nullptr;
                    }
                }
                break;
            case FrameType::HEADERS:
                {
                    ...
                    if(!frame->data->readFrom(ba, frame->header)) {
                        CHAT_LOG_INFO(g_logger) << "parse HeadersFrame fail";
                        return nullptr;
                    }
                }
                break;
            case FrameType::PRIORITY:
                {
                    ...
                    if(!frame->data->readFrom(ba, frame->header)) {
                        CHAT_LOG_INFO(g_logger) << "parse PriorityFrame fail";
                        return nullptr;
                    }
                }
                break;
            case FrameType::RST_STREAM:
                {
                    ...
                    if(!frame->data->readFrom(ba, frame->header)) {
                        CHAT_LOG_INFO(g_logger) << "parse RstStreamFrame fail";
                        return nullptr;
                    }
                }
                break;
            case FrameType::SETTINGS:
                {
                    ...
                    if(!frame->data->readFrom(ba, frame->header)) {
                        CHAT_LOG_INFO(g_logger) << "parse SettingsFrame fail";
                        return nullptr;
                    }
                }
                break;
            case FrameType::PUSH_PROMISE:
                {
                    ...
                    if(!frame->data->readFrom(ba, frame->header)) {
                        CHAT_LOG_INFO(g_logger) << "parse PushPromisedFrame fail";
                        return nullptr;
                    }
                }
                break;
            case FrameType::PING:
                {
                    ...
                    if(!frame->data->readFrom(ba, frame->header)) {
                        CHAT_LOG_INFO(g_logger) << "parse PingFrame fail";
                        return nullptr;
                    }
                }
                break;
            case FrameType::GOAWAY:
                {
                    ...
                    if(!frame->data->readFrom(ba, frame->header)) {
                        CHAT_LOG_INFO(g_logger) << "parse GoAwayFrame fail";
                        return nullptr;
                    }
                }
                break;
            case FrameType::WINDOW_UPDATE:
                {
                    ...
                    if(!frame->data->readFrom(ba, frame->header)) {
                        CHAT_LOG_INFO(g_logger) << "parse WindowUpdateFrame fail";
                        return nullptr;
                    }
                }
                break;
            ...
            default:
                {
                    CHAT_LOG_WARN(g_logger) << "invalid FrameType: " << (uint32_t)frame->header.type;
                    return nullptr;
                }
                break;
        }
        ...
        CHAT_LOG_ERROR(g_logger) << "FrameCodec parseFrom except: " << e.what();
    } catch(...) {
        CHAT_LOG_ERROR(g_logger) << "FrameCodec parseFrom except";
    }
    return nullptr;
}
...
int32_t FrameCodec::serializeTo(Stream::ptr stream, Frame::ptr frame) {
    ...
    if(frame->data) {
        if(!frame->data->writeTo(ba, frame->header)) {
            CHAT_LOG_ERROR(g_logger) << "FrameCodec serializeTo fail, type=" << FrameTypeToString((FrameType)frame->header.type);
            return -1;
        }
    ...
    if(rt <= 0) {
        CHAT_LOG_ERROR(g_logger) << "FrameCodec serializeTo fail, rt=" << rt << " errno=" << errno
            << " - " << strerror(errno);
        return -2;
    }
    return ba->getSize();
