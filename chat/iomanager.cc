...
IOManager::IOManager(size_t threads, bool use_caller, const std::string& name) 
    ...
    rt = fcntl(m_tickleFds[0], F_SETFL, O_NONBLOCK);  //设置非阻塞 可修改
    CHAT_ASSERT(!rt);
...
int IOManager::addEvent(int fd, Event event, std::function<void()> cb) {
    ...
    if (fd_ctx->m_events & event) {
        ...
        CHAT_ASSERT(!(fd_ctx->m_events & event));
    }
    ...
    if (rt) {
        CHAT_LOG_ERROR(g_logger) << "epoll_ctl(" << m_epfd << ", " << op << ", " << fd << ", " << epevent.events << "):" << rt << "(" << errno << ") (" << strerror(errno) << ")";
        return -1;
    }
...
bool IOManager::delEvent(int fd, Event event) {
    ...
    Event new_events = (Event)(fd_ctx->m_events & ~event);  //去掉event
...
bool IOManager::cancelAll(int fd) {
    ...
    int rt = epoll_ctl(m_epfd, op, fd, &epevent);
    if(rt) {
        CHAT_LOG_ERROR(g_logger) << "epoll_ctl(" << m_epfd << ", "
...
void IOManager::idle() {
    ...
    while (true) {
        ...
        if (stopping(next_timeout)) {  //获取了最近定时器时间
            CHAT_LOG_INFO(g_logger) << "name=" << getName() << " idle stopping exit";
            break;
        }
