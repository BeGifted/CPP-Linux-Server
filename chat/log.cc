...
void Logger::setFormatter(LogFormatter::ptr val) {
    ...
    m_formatter = val;
    for(auto& i : m_appenders) {
        LogAppender::MutexType::Lock lc(i->m_mutex);
        if(!i->m_hasFormatter) {
...
void Logger::addAppender(LogAppender::ptr appender) {
    ...
    if (!appender->getFormatter()) {
        LogAppender::MutexType::Lock lc(appender->m_mutex);
...
chat::ConfigVar<std::set<LogDefine> >::ptr g_log_defines =
    chat::Config::Lookup("logs", std::set<LogDefine>(), "logs config");
...
struct LogIniter {
    LogIniter() {
        g_log_defines->addListener([](const std::set<LogDefine>& old_v, const std::set<LogDefine>& new_v) {
            ...
            for (auto& i : old_v) {
                auto it = new_v.find(i);
                if (it == new_v.end()) {
                    //删除logger
                    auto logger = CHAT_LOG_NAME(i.name);
